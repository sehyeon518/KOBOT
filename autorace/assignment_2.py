...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        # 사다리꼴의 4개의 점 정의
        points = np.array([[0, HEIGHT], [300, 200], [WIDTH-300, 200], [WIDTH, HEIGHT]])

        # 사다리꼴 그리기
        cv2.polylines(edge, [points], isClosed=True, color=(0, 255, 0), thickness=2, lineType=cv2.LINE_AA)
        cv2.fillPoly(edge, [points], color=(0, 0, 0))

        # mask
        pt1 = 0,0
        pt2 = WIDTH, HEIGHT/2+50
        cv2.rectangle(edge, pt1, pt2, (0,0,0), -1)
